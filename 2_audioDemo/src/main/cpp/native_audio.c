 ...
 *  @date    2016/04/17
 */
#include "include/opensl_io.h"
#include <jni.h>
#include <stdio.h>
#include <android/log.h>

#define LOGI(...) __android_log_print(ANDROID_LOG_DEBUG,"AudioDemo-JNI",__VA_ARGS__)
...
Java_com_example_daixiansen_audiostudydemo_tester_NativeAudioTester_nativeStartCapture(JNIEnv *env,
                                                                                       jobject thiz) {
    ...
    if (stream == NULL) {
        fclose(fp);
        LOGI("failed to open audio device ! \n");
        return JNI_FALSE;
    }
    ...
    while (!g_loop_exit) {
        ...
        if (samples < 0) {
            LOGI("android_AudioIn failed !\n");
            break;
        }
        if (fwrite((unsigned char *) buffer, samples * sizeof(short), 1, fp) != 1) {
            LOGI("failed to save captured data !\n ");
            break;
        }
        LOGI("capture %d samples !\n", samples);
    }

    android_CloseAudioDevice(stream);
    fclose(fp);

    LOGI("nativeStartCapture completed !");
 ...
 */
JNIEXPORT jboolean JNICALL
Java_com_example_daixiansen_audiostudydemo_tester_NativeAudioTester_nativeStopCapture(JNIEnv *env,
                                                                                      jobject thiz) {
    g_loop_exit = 1;
    return JNI_TRUE;
}
 ...
 */
JNIEXPORT jboolean JNICALL
Java_com_example_daixiansen_audiostudydemo_tester_NativeAudioTester_nativeStartPlayback(JNIEnv *env,
                                                                                        jobject thiz) {
    ...
    while (!g_loop_exit && !feof(fp)) {
        ...
        if (samples < 0) {
            LOGI("android_AudioOut failed !\n");
        }
        LOGI("playback %d samples !\n", samples);
    }
 ...
 */
JNIEXPORT jboolean JNICALL
Java_com_example_daixiansen_audiostudydemo_tester_NativeAudioTester_nativeStopPlayback(JNIEnv *env,
                                                                                       jobject thiz) {
    g_loop_exit = 1;
    return JNI_TRUE;
}